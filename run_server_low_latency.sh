...
env_file="${OPENRECALL_ENV_FILE:-$repo_root/myrecall_server_low_latency.env}"
enable_debug="false"
...
for arg in "$@"; do
  case "$arg" in
    ...
    --env=*)
      ...
      ;;
    *)
      echo "Usage: $0 [--debug] [--env=/abs/path/to/myrecall_server_low_latency.env]" >&2
      exit 2
      ;;
  esac
done
...
echo "[run_server_low_latency] env_file=$env_file"
