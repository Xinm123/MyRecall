...
from myrecall.client.events.base import utc_now_iso
...
def detect_permissions() -> PermissionCheckResult:
    ...
    if os.environ.get("MYRECALL_SKIP_PERMISSION_CHECK", "").lower() == "true":
        if settings.debug:
            logger.debug(
                "DEV MODE: Permission bypass active (MYRECALL_SKIP_PERMISSION_CHECK=true)"
            )
