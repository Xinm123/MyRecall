"""MyRecall Client entry point.

Launch with: python -m myrecall.client [--config=/path/to/client.toml]
...
for arg in sys.argv[1:]:
    if arg == "--no-web":
        ...
        os.environ["MYRECALL_CLIENT_WEB_ENABLED"] = "false"
        sys.argv.remove(arg)
        break
...
def _parse_args():
    parser = argparse.ArgumentParser(prog="myrecall-client")
    parser.add_argument(
        "--config",
        type=str,
        default=None,
...
def main():
    """Start the MyRecall client (recorder + uploader)."""
    # Import here (not module-level) so Flask's debug-reload subprocess
    # does not hit settings before myrecall.shared.config.settings is set.
    from myrecall.client.recorder import get_recorder

    global logger
    logger = configure_logging("myrecall.client")
    logger.info("=" * 50)
    logger.info("MyRecall Client Starting")
    logger.info("=" * 50)
    ...
    if settings.client_web_enabled:
        from myrecall.client.web.app import start_web_server
