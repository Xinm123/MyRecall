...
Output:
{
  "narrative": "User is editing a Python file named api_v1.py in Visual Studio Code. The editor shows a FastAPI route definition for POST /v1/ingest with type hints and docstrings. The integrated terminal at the bottom displays a pytest run with 5 passing tests. The Explorer sidebar shows 8 files in the myrecall project. A git diff indicator shows 2 modified files.",
...
Output:
{
  "narrative": "User has a terminal window open in iTerm2. The current working directory is ~/projects/myrecall. The terminal shows output from a git status command indicating 3 modified files and 1 untracked file. The shell prompt shows the user is on branch 'feature/prompt-optimization'. The previous command was 'git diff myrecall/server/description/providers/openai.py'.",
  "summary": "Running git commands in terminal for myrecall project",
...
Output:
{"narrative":"User is editing a Python file named api_v1.py in Visual Studio Code. The editor shows a FastAPI route definition for POST /v1/ingest with type hints. The integrated terminal displays a pytest run with 5 passing tests. The Explorer sidebar shows 8 files in the myrecall project.","summary":"Editing FastAPI route and running tests in VS Code","tags":["vscode","python","fastapi","coding","testing","development"]}
...
Output:
{"narrative":"User has a terminal window open in iTerm2. The working directory is ~/projects/myrecall. The terminal shows git status output with 3 modified files and 1 untracked file. The shell prompt shows branch 'feature/prompt-optimization'.","summary":"Running git commands in terminal for myrecall project","tags":["terminal","iterm2","git","command-line","development"]}
