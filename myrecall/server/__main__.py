"""MyRecall Server entry point.

Launch with: python -m myrecall.server [--config=/path/to/server.toml]
"""
...
def _parse_args():
    parser = argparse.ArgumentParser(prog="myrecall-server")
    parser.add_argument(
        "--config",
        type=str,
        default=None,
...
myrecall.shared.config.settings = settings
...
def preload_ai_models():
    ...
    try:
        ...
        if ocr_provider == "rapidocr" and settings.ocr_rapid_use_local:
            ...
            from myrecall.server.ocr.rapid_backend import RapidOCRBackend
...
def _start_noop_mode():
    ...
    from myrecall.server.queue_driver import NoopQueueDriver
    ...
    if settings.description_enabled:
        ...
        from myrecall.server.description.worker import DescriptionWorker
...
def main():
    global logger
    logger = configure_logging("myrecall.server")
    logger.info("=" * 50)
    logger.info("MyRecall Server Starting")
    logger.info("=" * 50)
    ...
    from myrecall.server.runtime_config import init_runtime_config
    ...
    if processing_mode == "noop":
        ...
        preload_ai_models()
        from myrecall.server.app import init_background_worker
