...
from myrecall.server.database import SQLStore
from myrecall.server.ai.base import AIProviderError
from myrecall.server.ai.factory import (
    get_ai_provider,
...
from myrecall.shared.models import RecallEntry

# Phase 3 Imports
from myrecall.server.utils.keywords import KeywordExtractor
from myrecall.server.utils.fusion import build_fusion_text
from myrecall.server.schema import SemanticSnapshot, Context, Content
from myrecall.server.database.vector_store import VectorStore
...
class ProcessingWorker(threading.Thread):
    ...
    def run(self):
        ...
        logger.info(
            ...
            f"vision={settings.ai_provider}, "
            f"ocr={settings.ocr_provider or settings.ai_provider}, "
            f"embedding={settings.ai_provider}, "
        ...
        try:
            while not self._stop_event.is_set():
                try:
                    ...
                    pending_count = (
                        sql_store.get_pending_count(conn) if sql_store else 0
                    )
                    ...
                    # Mark as PROCESSING
                    if not (
                        sql_store and sql_store.mark_task_processing(conn, task.id)
                    ):
    ...
    def _process_task(
        ...
        try:
            ...
            if settings.debug:
                logger.debug(
                    f"  AI: {len(caption)} chars description. Tags: {scene}, {action}"
                )
            ...
            try:
                ...
                logger.warning(
                    f"⚠️ Task #{task.id}: Embedding provider unexpected error: {e}"
                )
                embedding = np.zeros(int(settings.embedding_dim), dtype=np.float32)
                snapshot.embedding_vector = embedding.tolist()
            ...
            if sql_store:
                success = sql_store.mark_task_completed(
                    conn, task.id, text, caption, embedding
                )

            if success:
                if settings.debug:
                    logger.info(
                        f"✅ Task #{task.id} completed successfully (Phase 3 Pipeline)"
                    )
            else:
                if should_cancel():
                    if sql_store:
