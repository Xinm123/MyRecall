"""Configuration management for MyRecall using pydantic-settings."""
...
warnings.warn(
    "myrecall.shared.config is deprecated. Use TOML-based configuration instead. "
...
class Settings(BaseSettings):
    ...
    - MYRECALL_DEBUG: Enable debug mode (verbose logging)
    - MYRECALL_DATA_DIR: Base directory for all data storage
    - MYRECALL_PORT: Web server port
    - MYRECALL_PRIMARY_MONITOR_ONLY: Only record primary monitor
    - MYRECALL_API_URL: Server API URL for client communication
    - MYRECALL_CAPTURE_INTERVAL: Screenshot capture interval in seconds
    - MYRECALL_DEVICE: Device for AI inference (cpu, cuda, mps)
    - MYRECALL_AI_PROVIDER: AI provider for vision analysis (local, dashscope, openai)
    - MYRECALL_AI_MODEL_NAME: Model name/path for the selected AI provider
    - MYRECALL_AI_API_KEY: API key for cloud AI providers
    - MYRECALL_AI_API_BASE: Optional base URL for OpenAI-compatible proxies
    - MYRECALL_VISION_PROVIDER: Optional override for vision provider (falls back to MYRECALL_AI_PROVIDER)
    - MYRECALL_VISION_MODEL_NAME: Optional override for vision model name/path
    - MYRECALL_VISION_API_KEY: Optional override for vision API key
    - MYRECALL_VISION_API_BASE: Optional override for vision API base URL
    - MYRECALL_OCR_PROVIDER: Optional override for OCR provider (falls back to MYRECALL_AI_PROVIDER)
    - MYRECALL_OCR_MODEL_NAME: Optional override for OCR model name (for API-based OCR)
    - MYRECALL_OCR_API_KEY: Optional override for OCR API key
    - MYRECALL_OCR_API_BASE: Optional override for OCR API base URL
    - MYRECALL_EMBEDDING_PROVIDER: Optional override for embedding provider (falls back to MYRECALL_AI_PROVIDER)
    - MYRECALL_EMBEDDING_MODEL_NAME: Optional override for embedding model name (for API embeddings)
    - MYRECALL_EMBEDDING_API_KEY: Optional override for embedding API key
    - MYRECALL_EMBEDDING_API_BASE: Optional override for embedding API base URL
    - MYRECALL_UPLOAD_TIMEOUT: Client upload timeout in seconds
    - MYRECALL_EMBEDDING_MODEL: Embedding model name for semantic search
    - MYRECALL_CLIENT_WEB_PORT: Port for client web UI server
    - MYRECALL_CLIENT_WEB_ENABLED: Enable client web UI server
    - MYRECALL_EDGE_BASE_URL: Base URL for Edge API server (used by client web UI)
    - MYRECALL_CLIENT_CORS_ORIGIN: Allowed CORS origin for Edge server
    """

    debug: bool = Field(default=True, alias="MYRECALL_DEBUG")
    capture_interval: int = Field(default=10, alias="MYRECALL_CAPTURE_INTERVAL")

    # Three-layer debounce configuration (separate control for each layer)
    click_debounce_ms: int = Field(
        default=3000,
        alias="MYRECALL_CLICK_DEBOUNCE_MS",
    ...
    stats_interval_sec: int = Field(
        default=120,
        alias="MYRECALL_STATS_INTERVAL_SEC",
        description="Statistics logging interval in seconds",
    )
    host: str = Field(default="127.0.0.1", alias="MYRECALL_HOST")
    port: int = Field(default=8083, alias="MYRECALL_PORT")
    primary_monitor_only: bool = Field(
        default=True, alias="MYRECALL_PRIMARY_MONITOR_ONLY"
    )

    # Split Data Directories
    server_data_dir: Path = Field(
        default_factory=lambda: Path.home() / ".myrecall" / "server",
        alias="MYRECALL_SERVER_DATA_DIR",
    ...
    client_data_dir: Path = Field(
        ...
        alias="MYRECALL_CLIENT_DATA_DIR",
    ...
    # We keep it as a field because Pydantic might try to populate it from env MYRECALL_DATA_DIR
    base_path_legacy: Optional[Path] = Field(default=None, alias="MYRECALL_DATA_DIR")

    cache_dir: Optional[Path] = Field(
        default=None,
        alias="MYRECALL_CACHE_DIR",
    ...
    ocr_rapid_ocr_version: str = Field(
        default="PP-OCRv4",
        alias="MYRECALL_OCR_RAPID_OCR_VERSION",
    ...
    ocr_rapid_model_type: str = Field(
        default="mobile",
        alias="MYRECALL_OCR_RAPID_MODEL_TYPE",
    ...
    ocr_det_db_thresh: float = Field(
        default=0.3,
        alias="MYRECALL_OCR_DET_DB_THRESH",
        description=(
            "DB检测算法的像素级文本概率阈值。"
    ...
    ocr_det_db_box_thresh: float = Field(
        default=0.7,
        alias="MYRECALL_OCR_DET_DB_BOX_THRESH",
        description=(
            "DB检测算法的文本框置信度阈值。"
    ...
    ocr_det_db_unclip_ratio: float = Field(
        default=1.6,
        alias="MYRECALL_OCR_DET_DB_UNCLIP_RATIO",
        description=(
    ...
    ocr_det_db_score_mode: str = Field(
        default="slow",
        alias="MYRECALL_OCR_DET_DB_SCORE_MODE",
        description=(
            "DB检测算法的置信度计算方式。"
    ...
    ocr_det_limit_side_len: int = Field(
        default=960,
        alias="MYRECALL_OCR_DET_LIMIT_SIDE_LEN",
        description=(
            "检测图像边长限制。"
    ...
    description_model: Optional[str] = Field(
        default=None,
        alias="MYRECALL_DESCRIPTION_MODEL",
    ...
    description_api_key: Optional[str] = Field(
        default=None,
        alias="MYRECALL_DESCRIPTION_API_KEY",
    ...
    description_api_base: Optional[str] = Field(
        default=None,
        alias="MYRECALL_DESCRIPTION_API_BASE",
    ...
    processing_mode: str = Field(
        default="ocr",
        alias="MYRECALL_PROCESSING_MODE",
    ...
    disable_similarity_filter: bool = Field(
        default=False,
        alias="MYRECALL_DISABLE_SIMILARITY_FILTER",
    ...
    simhash_dedup_enabled: bool = Field(
        default=True,
        alias="MYRECALL_SIMHASH_DEDUP_ENABLED",
    ...
    simhash_dedup_threshold: int = Field(
        ...
        ge=0,
        le=64,
        alias="MYRECALL_SIMHASH_DEDUP_THRESHOLD",
    ...
    simhash_ttl_seconds: float = Field(
        default=60.0,
        alias="MYRECALL_SIMHASH_TTL_SECONDS",
    ...
    client_save_local_screenshots: bool = Field(
        default=False,
        alias="MYRECALL_CLIENT_SAVE_LOCAL_SCREENSHOTS",
    ...
    def _ensure_dirs_on_init(self) -> "Settings":
        ...
        if "MYRECALL_IDLE_CAPTURE_INTERVAL_MS" not in os.environ:
            legacy_capture_interval = os.environ.get("MYRECALL_CAPTURE_INTERVAL")
...
# Entry points MUST set myrecall.shared.config.settings to the real settings object
# (ServerSettings.from_toml() or ClientSettings.from_toml()) BEFORE importing modules that
# use `from myrecall.shared.config import settings`.
    ...
    def __getattr__(self, name: str) -> Any:
        if _SettingsProxy._instance is None:
            raise AttributeError(
                ...
                "and assign to myrecall.shared.config.settings BEFORE importing "
                "modules that use `from myrecall.shared.config import settings`."
            )
...
def __getattr__(name: str) -> Any:
    ...
    raise AttributeError(f"module 'myrecall.shared.config' has no attribute '{name}'")
