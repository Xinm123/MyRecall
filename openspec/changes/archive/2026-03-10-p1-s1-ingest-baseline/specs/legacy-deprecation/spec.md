...
### Requirement: legacy 端点重定向（POST=308, GET=301）

Edge SHALL 对以下 4 个 legacy 端点返回重定向，响应头包含 `Location` 指向对应的 `/v1/*` 路径：
...
- **THEN** Edge 返回 `308`，响应头 `Location` 值为 `/v1/ingest`
...
### Requirement: legacy 重定向仅为迁移提示，不依赖 redirect 作为功能路径

P1-S1 的 legacy 重定向仅用于"废弃回归检查"。Host 主路径 SHALL 直接调用 `/v1/*`，不依赖 legacy 端点的 redirect 行为。

#### Scenario: 重定向不等价于目标端点可用
...
- **THEN** Edge 不返回重定向，按现有逻辑处理（可能返回 200 或 404，取决于现有实现）

### Requirement: P1-S4 起切换为 410 Gone

自 P1-S4 起，上述 4 个 legacy 端点 SHALL 从重定向切换为 `410 Gone`（完全废弃）。P1-S1 至 P1-S3 期间 SHALL 保持：`POST /api/upload` 返回 `308`，其余 3 个 GET 端点返回 `301`。

#### Scenario: P1-S1 阶段返回 legacy 重定向

- **WHEN** P1-S1 阶段客户端请求任一 legacy 端点
- **THEN** 返回约定重定向（`POST /api/upload` 为 `308`，其余 GET 为 `301`），而非 `410`
