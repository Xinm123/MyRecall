...
- legacy 命名空间渐进废弃（P1-S1~S3）：仅对 Gate scope 内的 4 个 legacy 端点返回重定向并带 `Location` 指向对应 `/v1/*`，并记录 `[DEPRECATED]` 日志：`POST /api/upload` 返回 `308`，`GET /api/search`、`GET /api/queue/status`、`GET /api/health` 返回 `301`
  - ⚠️ 说明：legacy 重定向是"迁移公告"，不等价于"目标端点当期已完成实现"。S1 阶段 `/v1/search` 可能返回 404（属预期，P1-S4 才实现）。
...
- `legacy-deprecation`: `/api/*`（仅 Gate scope 4 个端点）到 `/v1/*` 的重定向规则（POST=308, GET=301）+ `Location` + `[DEPRECATED]` 与阶段演进（P1-S4 起切换为 410）。
...
- API 面：对外默认入口从 `/api/*` 收敛到 `/v1/*`；legacy 端点仅保留迁移提示重定向（`POST /api/upload`=308，其余 GET=301），并明确不实现 catch-all `/api/*`。
