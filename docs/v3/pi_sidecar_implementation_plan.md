...
- 关联文档：`ADR-0004`、[spec.md](./spec.md) §4.6、[roadmap.md](./roadmap.md) P1-S5/S6/S7、`baselines/chat/chat_baseline_myrecall.md`
...
| # | 交付物 | 文件 | 估算行数 | 说明 |
|---|--------|------|---------|------|
| S5-1 | bun + Pi 安装与检测 | `chat/config.py`（扩展） | ~100 | bundled bun 查找顺序：1) 可执行文件同目录的捆绑 bun → 2) ~/.bun/bin/bun → 3) 系统路径；检测 Pi 可用性；缺失时尝试自动安装（`bun add -g @mariozechner/pi-coding-agent@<pinned-version>`，对齐 screenpipe）；`/v1/chat` 端点返回 Pi 可用性状态 |
