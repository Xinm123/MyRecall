...
- 版本：v1.2
- 日期：2026-03-03
- 依赖决策（DEC-*）：DEC-001A、DEC-002A (Revised 2026-03-01)、DEC-005A (Revised 2026-02-26)、DEC-013A
- 实现约束（非决策 ID）：IMP-1（Flask + threading）、IMP-2（Edge 数据目录）、IMP-3（Manager 始终从 chat_messages 注入历史）
- 关联文档：[`ADR-0004`](./adr/ADR-0004-chat-rag-orchestration-on-edge.md)、[`api_contract.md`](./api_contract.md)（API-500）、[`data_model.md`](./data_model.md)（DB-001，`chat_messages`）、[`decisions.md`](./decisions.md)（DEC-002A/DEC-005A/DEC-013A）、[`roadmap.md`](./roadmap.md)（P1-S5/S6/S7）、[`baselines/chat/chat_baseline_myrecall.md`](./baselines/chat/chat_baseline_myrecall.md)

> **决策修订说明**：
> - DEC-002A：2026-03-01 修订，Chat 请求改为简单 JSON，响应改为 SSE 透传 Pi 原生事件
> - DEC-005A：2026-02-26 修订，Edge 支持 local/cloud 双 provider，P1 不做自动 fallback
> 
> 完整决策状态以 [`decisions.md`](./decisions.md) 为准。
...
Frontend (Alpine.js + EventSource)
  ...
  ▼
Edge Python (Flask + threading, IMP-1)
  ...
  │    ├─ session history injection (IMP-3: 始终从 chat_messages 注入)
...
| S5-3 | PiManager | `chat/manager.py` | ~400 | Singleton；`start()`/`stop()`/`send_prompt(message, session_id, images?)`/`abort(req_id)`/`new_session()`；Session history injection（IMP-3：每次 `send_prompt` 前从 `chat_messages` 读取最近 N 条，构造 `<conversation_history>` XML block 拼入 prompt）；Event queue（`queue.Queue`）供 SSE generator 消费 |
...
| S5-6 | Skills 注入 | `chat/config.py` | ~60 | `inject_skills(pi_workdir)`：将 `openrecall/server/skills/*/SKILL.md` 复制到 `{pi_workdir}/skills/{name}/SKILL.md`；`detect_bun()`/`detect_pi()`：检查可执行文件可用性；`get_pi_workdir() → Path`：返回 `$OPENRECALL_SERVER_DATA_DIR/.pi`（IMP-2） |
| S5-7 | /v1/chat endpoint | `api.py`（修改） | ~80 | `POST /v1/chat`：解析 `{message, session_id, images?}`；调用 `PiManager.send_prompt()`；返回 `Response(stream_with_context(sse_generator()), mimetype='text/event-stream')`（IMP-1：Flask + threading） |
| S5-8 | 持久化 | `chat/persistence.py` | ~80 | `save_message(session_id, role, content, citations, tool_calls, model, latency_ms)`；`get_session_history(session_id, limit=20) → list[dict]`；`list_sessions() → list[dict]`；使用 `chat_messages` 表（schema per `data_model.md` DB-001，Table 5）；stream end 后由 Manager 调用保存 user + assistant 消息 |
...
| S6-5 | Provider timeout | `chat/manager.py`（扩展） | ~40 | First-token timeout >15s → abort 当前请求 → 返回 timeout error event；**不做自动 fallback**（DEC-005A，对齐 screenpipe）；日志记录 timeout 上下文 |
...
| S7-2 | Citation 覆盖率评估 | 评估报告（非代码） | — | 在 grounding 问题集（>= 80 条）上统计引用覆盖率；评估当前提示词驱动 citation 策略的实际效果 |
| S7-3 | 结构化 citation B 阶段决策 | ADR 更新或新建 | — | 根据 S7-2 评估结果决定：是否需要结构化 citation 后处理（见 OQ-028）；若需要，产出实现方案 |
...
| 实现约束 | 选项 | 确认结果 | 理由 |
|--------|------|---------|------|
| IMP-1 | Flask SSE 流式方案 | **A) Flask + threading** | P1 单用户场景足够；`stream_with_context` + daemon thread 读取 Pi stdout → queue → SSE generator；避免 async 迁移成本 |
| IMP-2 | Pi 工作目录 | **A) Edge 数据目录** `$OPENRECALL_SERVER_DATA_DIR/.pi/` | 与 MyRecall 数据管理统一；Pi skills、models.json、auth.json 均在 Edge 数据目录下；备份/迁移时随数据目录一起 |
| IMP-3 | Session 历史注入 | **A) Manager 始终注入** | 每次 `send_prompt` 前从 `chat_messages` 读取最近 N 条历史拼入 prompt；Pi 进程重启/crash 后自动恢复对话上下文；不依赖 Pi 内存状态 |
...
| Flask + threading 在高并发下阻塞 | P1 不受影响（单用户） | P1 scope 明确单用户；P2+ 若需多用户并发再评估 async 迁移（IMP-1 的替代方案） |
| 提示词驱动 citation 覆盖率不达标 | 引用质量低 | P1-S7 评估点决定是否启动结构化 citation B 阶段（见 OQ-028）；Skill 提示词持续优化；覆盖率为 non-blocking KPI |
...
| `chat/manager.py` (PiManager) | `pi.rs` PiManager struct | 行为对齐，增加 IMP-3 历史注入 |
...
| Flask SSE streaming | Tauri IPC events | 拓扑适配（per DEC-001A） |
