...
- 事件去抖与采样门控生效：满足 `min_capture_interval_ms=200` 口径并通过去抖违规 SQL 检查。
...
- P1-S2 新增测试集合（开发期间持续补充）：
  - `pytest tests/test_p1_s2_*.py -q`
  - 最小通过线：`0 failed`。
- 组合回归（阶段收口）：
  - `pytest tests/test_v3_migrations_bootstrap.py tests/test_p1_s1_*.py tests/test_p1_s2_*.py tests/test_p1_s1_grid_data_contract.py -q`
...
- 任一改动导致关键 HTTP 契约返回码/字段语义偏离 `docs/v3/spec.md` 与 openspec 记录。
