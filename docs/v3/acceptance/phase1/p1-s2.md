...
- 触发器约束：P1 `capture_trigger` 枚举以 `API-100` / `CapturePayload` 为准（`idle/app_switch/manual/window_focus/click`）；`typing_pause` 属于 P2+，不纳入 P1 Gate。
- 对应 Gate 条件：
...
- 运行环境：同 P1-S1。
- 配置与数据集：
  - 触发事件脚本（app switch/click/manual/window focus/idle）
  - 多窗口场景数据 30 分钟
- 依赖版本：记录客户端触发模块版本
