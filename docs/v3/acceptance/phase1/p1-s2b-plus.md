...
- 阶段：P1-S2b+
- 日期：2026-03-15（P1-S2b+ 感知哈希实现 - 就绪待执行）
- 负责人：pyw
- 状态：`Ready`（前置条件已满足，待实施）
- 依赖：P1-S2b Pass
...
| `simhash_heartbeat_interval_sec` | `300` | `OPENRECALL_SIMHASH_HEARTBEAT_SEC` | 强制心跳入库的最大静止时间阈值（秒）。超时后无论内容是否相似，都会强制写入下一帧作为时间线断点。 |
...
class SimhashCache:
    """Host 端 simhash 缓存，用于相似帧判定与长静止期兜底。
    
    - 按 device_name 分组存储
    - 每个 device 仅保留最近 1 帧及其入库时间戳
    - 进程重启后缓存清空（可接受）
    """
    
    def __init__(self, cache_size_per_device: int = 1):
        self._cache_size = cache_size_per_device
        self._cache: dict[str, deque[tuple[int, float]]] = defaultdict(lambda: deque(maxlen=cache_size_per_device))
    
    def add(self, device_name: str, simhash: int, enqueue_ts: float | None = None) -> None:
        """添加新帧的 simhash 和入库时间到缓存"""
        if enqueue_ts is None:
            enqueue_ts = time.time()
        self._cache[device_name].append((simhash, enqueue_ts))
    
    def is_similar_and_should_skip(self, device_name: str, simhash: int, threshold: int = 8, heartbeat_interval: float = 300.0, current_ts: float | None = None) -> tuple[bool, int | None]:
        """检查是否与缓存中的帧相似，同时判断是否达到心跳兜底阈值。
        
        Returns:
            (should_skip, similar_to_hash): 若相似且未达到心跳阈值则返回 (True, 相似的 hash 值)；否则返回 (False, None)
        """
        if current_ts is None:
            current_ts = time.time()
            
        for cached_hash, last_enqueue_ts in self._cache[device_name]:
            if hamming_distance(simhash, cached_hash) <= threshold:
                # 即使高度相似，若距离上次入库时间已超过心跳阈值，则不丢弃（强制入库以更新心跳）
                if current_ts - last_enqueue_ts >= heartbeat_interval:
                    return False, None
...
2. 计算 simhash（PHash，64-bit）
3. 查询 SimhashCache：
   - 若与缓存中的帧汉明距离 <= 8 bits 且 距离上次入库时间 < 心跳阈值（默认 300s） → 跳过入库，记录日志
   - 否则（不相似，或已超心跳阈值） → 将 simhash 及当前时间戳写入缓存，正常入库
4. 日志记录：
   - 跳过：`MRV3 similar_frame_skipped device_name=X hamming_distance=N trigger_type=Y trigger_time=Z`
   - 入库：`MRV3 frame_enqueued device_name=X simhash=0x...`
```
...
**说明**：
- 状态栏 `⬜` 表示待实施后填写
- 所有 Hard Gate 达标方可判定阶段 Pass
- 性能指标已移至 §4.2 Soft KPI，不作为 Pass/Fail 条件
