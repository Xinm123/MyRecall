...
| **B4**  | **工具以 Pi SKILL.md 格式定义**（对齐 screenpipe DA-3），P1-S5 最小集为 `myrecall-search` Skill（对标 `screenpipe-search`），通过 `curl` 调用 Edge `/v1/search` FTS 端点。统一包含搜索、时间范围渐进扩展、帧详情获取等能力。                                                                                                                                                          | `spec.md` §4.6; `ADR-0004`（修订）                                   |
