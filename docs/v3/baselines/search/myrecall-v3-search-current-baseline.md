...
> 生成日期：2026-03-02
> SSOT：docs/v3/decisions.md + docs/v3/api_contract.md + docs/v3/data_model.md + ADR-0005 + 当前代码库
> 链路标记：仅屏幕
...
[E-13] api_contract.md（API-200） | /v1/search 参数与响应契约 | H | 已核查
...
[E-14] data_model.md（DB-001） | DDL + FTS 触发器 | H | 已核查
...
data_model.md（DB-001，Scheme C 搜索路由）。Scheme C（025A）将搜索拆为三条独立路径，由 `content_type` 参数路由：
...
规范位置：`data_model.md`（DB-001，FTS 查询规范化）。

### 1.6 COUNT 查询（阶段0 新增）

D3=A 决策。独立 `COUNT(DISTINCT frames.id)` 支撑 `pagination.total`。
规范位置：`data_model.md`（DB-001，COUNT 查询与 Scheme C 三路径）。
