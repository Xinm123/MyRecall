...
[E-13] spec.md:859-877 | /v1/search 13 参数 | H | 已核查
...
[E-14] spec.md:87-288 | DDL + FTS 触发器 | H | 已核查
...
spec.md:452-578。Scheme C（025A）将搜索拆为三条独立路径，由 `content_type` 参数路由：
...
spec 已补充 §3.0.3（FTS 查询规范化）。

### 1.6 COUNT 查询（阶段0 新增）

D3=A 决策。独立 `COUNT(DISTINCT frames.id)` 支撑 `pagination.total`。
spec 已补充 §3.0.3（COUNT 查询，Scheme C 三路径）。
