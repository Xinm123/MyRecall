...
```sql
-- 骨架（无条件 INNER JOIN，对齐 screenpipe line 2117-2119）
...
**注**：screenpipe 明确注释"Avoid LEFT JOIN ocr_text — it forces a scan of the entire ocr_text"（db.rs line 3133）。v3 遵循同样规则，主搜索路径不使用 LEFT JOIN。
