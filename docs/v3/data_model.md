# MyRecall-v3 数据模型规范（SSOT）

- 版本：v1.0
- 日期：2026-03-03
- 作用：本文件是 v3 数据结构、DDL、搜索 SQL 路由与 migration 规则的唯一事实源。
- 关联：[`spec.md`](./spec.md), [`architecture.md`](./architecture.md), [`api_contract.md`](./api_contract.md), [`decisions.md`](./decisions.md)

## DB-000 范围

- 覆盖：表结构、索引、FTS、触发器、查询路由、迁移策略。
- 不覆盖：API 请求/响应格式（见 `api_contract.md`）、阶段计划（见 `roadmap.md`）。

## DB-001 规范正文（从原 spec §3 迁移）
### DB-001.1 数据模型总览（Edge SQLite，主路径对齐 screenpipe vision-only，差异显式）
...
**注**：各路径分别拉取 `limit+offset` 行（`offset=0`），最坏情况内存中合并 `2×(limit+offset)` 行。P1 场景 `limit≤100`、`offset` 通常小，内存可控——前提是 UI 采用"加载更多"而非跳页（`API-200` 已定义 `offset` 在 P1 为加载更多语义，对应 `DEC-020A`）。screenpipe UI 为纯"加载更多"模式（`search-modal.tsx: hasMoreOcr/loadMoreOcr`），offset 单调递增步长=limit，实际不超过几百。P2+ 可升级为 keyset cursor 分页（`before_timestamp`），彻底消除过量拉取，届时需改 API 契约。对齐 screenpipe `db.rs:1876-1880, 2043-2050`。
...
- 非 SSOT 文档（`spec.md`, `roadmap.md`, `open_questions.md`, `acceptance/**`）禁止复制 DDL/触发器/SQL 细节。
- 需要引用数据模型时，必须使用 DB-ID + 链接到本文件对应章节。
