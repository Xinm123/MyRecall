...
- [ ] @mention 正确过滤时间范围
- [x] 错误场景有友好提示
- [x] 用户可以配置 LLM Provider 和 API Key
...
| 2026-03-26 | Phase 3 完成，Web UI 验收通过，所有测试通过 |
| 2026-03-26 | Phase 4.1 会话持久化验收通过（已在 Phase 3 实现） |
| 2026-03-26 | Phase 4.4 配置集成完成：Settings Modal UI + auth.json 写入 + Provider/Model 选择 + 自动重启 Pi |
| 2026-03-26 | Phase 4.3 错误处理完成：增强错误类型 + 内联操作按钮 + API Key 检测 |
| 2026-03-26 | Phase 4 代码审查：修复测试导入错误、类型注解、Provider 验证 |
