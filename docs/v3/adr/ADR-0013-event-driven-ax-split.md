...
补充：由于 MyRecall 拆分为 S2b->S3 handoff，Host upload contract 比 screenpipe 内部 `Option` 语义更严格：`accessibility_text` 为 required string（允许 `""`），`content_hash` 为 required nullable string（允许 `null`）。
...
| Gate | 原类型 | 调整后 | 理由 |
|------|--------|--------|------|
| `dedup_skip_rate >= 95%` | Hard Gate | **移出正式验收** | Host 侧运行态计数跨重启不可比；dedup 效果改由 `inter_write_gap_sec` Hard Gate 覆盖 |
...
- P1-S2a 背压放行：`queue_saturation_ratio <= 10%`、`overflow_drop_count = 0`；`collapse_trigger_count` 仅作为观测指标记录
- P1-S2b Gate：content_hash 覆盖率 >= 90%（基于 raw `accessibility_text` 非空分母）；`inter_write_gap_sec`（Soft KPI + Hard Gate）
