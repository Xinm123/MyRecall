...
| 类型 | Contract-ID | 接口 | 变化/说明 | SSOT |
|---|---|---|---|---|
| CHANGE | C-API-INGEST-001B | POST `/v1/ingest` | metadata/CapturePayload 在 S2b 增加 `accessibility_text` 与 `content_hash` 字段；`accessibility_text` 为 required string（允许 `""`，禁止 `null`），`content_hash` 为 required nullable string（允许 `null`，禁止 `""`）；`text_source` 仍属于 P1-S3 处理语义，不在本阶段 delta 内 | [spec.md](spec.md) §4.7；[data-model.md](data-model.md) §3.0.6；`acceptance/phase1/p1-s2b.md` §1.1 |
