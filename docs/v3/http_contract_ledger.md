...
| CHANGE | C-API-QUEUE-002A | GET `/v1/ingest/queue/status` | S2a 增加 `status_sync` 子结构：状态同步延迟统计（pending→completed），含 p50/p90/p95/p99/sample_count/window_id/edge_pid/broken_window | [spec.md](spec.md) §4.8.1 |
| CHANGE | C-API-HEALTH-001A | GET `/v1/health` | S2a+ 权限契约收口：响应必须包含 `capture_permission_status`、`capture_permission_reason`、`last_permission_check_ts`；权限失效/恢复中不得返回 `status=ok`；权限快照超时返回 `stale_permission_state` | [spec.md](spec.md) §4.9；`acceptance/phase1/p1-s2a-plus.md` |

### P1-S2b delta（Capture Completion / Monitor-Aware Coordination）

| 类型 | Contract-ID | 接口 | 变化/说明 | SSOT |
|---|---|---|---|---|
| CHANGE | C-API-INGEST-001B | POST `/v1/ingest` | S2b 不新增 OCR-only 主线路径必填字段；对外 HTTP 在本阶段主要收口 capture completion 语义（`device_name` 绑定、focused context same-cycle coherence、spool/ingest handoff 一致性）。`accessibility_text` / `content_hash` 若出现在 wire 上，仅属兼容/预留字段，不构成 v3 Gate 前提 | [roadmap.md](roadmap.md) §1.1；[spec.md](spec.md) §4.7；[data-model.md](data-model.md) §3.0.6；`acceptance/phase1/p1-s2b.md` §1.1 |

### P1-S3 delta（处理）

| 类型 | Contract-ID | 接口 | 变化/说明 | SSOT |
|---|---|---|---|---|
| CHANGE | C-API-INGEST-002B | GET `/v1/ingest/queue/status` | `processing_mode` 语义变更：P1-S1 为 `"noop"` → P1-S3+ 为 `"ocr"`（启用 OCR 处理与 `ocr_text` 持久化） | [spec.md](spec.md) §4.7 |
| RETAIN | (others) | `/v1/*` | 对外 HTTP 无新增/废弃/替代端点 | [spec.md](spec.md) §4.9 |

### P1-S4 delta（检索）

| 类型 | Contract-ID | 接口 | 变化/说明 | SSOT |
|---|---|---|---|---|
| ADD | C-API-SEARCH-001 | GET `/v1/search` | Search 对外可用（FTS5+过滤，v3 主线按 OCR-only 解释） | [spec.md](spec.md) §4.5 |
...
| ADD | C-API-FRAME-003 | GET `/v1/frames/:frame_id/context` | 端点对外可用（HTTP-only：frame context JSON；v3 主线返回 OCR-derived context） | [spec.md](spec.md) §4.9 |
