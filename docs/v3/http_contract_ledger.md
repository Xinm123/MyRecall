...
- 不覆盖：Pi 内部 JSON Lines / SSE 事件类型细节、deep link 语义、DB schema 细节（它们在 `spec.md` / `data-model.md` 中仍是 SSOT，但不在本 ledger 的“HTTP-only scope”里细拆）。

## 1. SSOT（唯一事实源）

- P1 端点清单：`MyRecall/docs/v3/spec.md` §4.9
- 各端点完整契约：`MyRecall/docs/v3/spec.md`（search/chat/ingest/frames/health 各节）
- Phase 1 子阶段与 `/api/*` 渐进策略：`MyRecall/docs/v3/roadmap.md` §1.1
- 决策记录：`MyRecall/docs/v3/open_questions.md`（OQ-024 等）
...
说明：详细 request/response schema 以 `spec.md` 为准，本文件只做索引与阶段映射。

## 3. Phase 1 HTTP Baseline（End State）

P1 的最终对外 HTTP surface（端点存在性与对外命名空间）以 `spec.md` §4.9 为准：

| Contract-ID | Method | Path | SSOT |
|---|---|---|---|
| C-API-INGEST-001 | POST | `/v1/ingest` | `spec.md` §4.7 |
| C-API-INGEST-002 | GET | `/v1/ingest/queue/status` | `spec.md` §4.7 |
| C-API-SEARCH-001 | GET | `/v1/search` | `spec.md` §4.5 |
| C-API-CHAT-001 | POST | `/v1/chat` | `spec.md` §4.6/§4.9 |
| C-API-FRAME-001 | GET | `/v1/frames/:frame_id` | `spec.md` §4.9 |
| C-API-FRAME-002 | GET | `/v1/frames/:frame_id/metadata` | `spec.md` §4.9 |
| C-API-FRAME-003 | GET | `/v1/frames/:frame_id/context` | `spec.md` §4.9 |
| C-API-HEALTH-001 | GET | `/v1/health` | `spec.md` §4.9 |
...
SSOT：`spec.md` §4.9（统一错误响应格式）。

## 4. Legacy namespace retirement（`/api/*`）

SSOT：`roadmap.md` §1.1；`spec.md` §4.5（命名空间冻结）；`open_questions.md` OQ-024。
...
| 类型 | Contract-ID | 接口 | 变化/说明 | SSOT |
|---|---|---|---|---|
| CHANGE | C-API-INGEST-001 | POST `/v1/ingest` | metadata/CapturePayload 字段口径在 P1 生效：`capture_trigger` 枚举为 `idle/app_switch/manual/click`（`window_focus` 不纳入 P1） | `spec.md` §4.7；`data-model.md` §3.0.6 |

### P1-S3 delta（处理）

| 类型 | Contract-ID | 接口 | 变化/说明 | SSOT |
|---|---|---|---|---|
| RETAIN | (all) | `/v1/*` | 对外 HTTP 无新增/废弃/替代端点（处理阶段主要影响内部数据与可解释性） | `spec.md` §4.9 |
...
| 类型 | Contract-ID | 接口 | 变化/说明 | SSOT |
|---|---|---|---|---|
| ADD | C-API-CHAT-001 | POST `/v1/chat` | Chat 对外可用（HTTP-only：JSON request + SSE response） | `spec.md` §4.6/§4.9 |
| ADD | C-API-FRAME-002 | GET `/v1/frames/:frame_id/metadata` | 端点对外可用（HTTP-only：最小稳定字段 `{frame_id,timestamp}`） | `spec.md` §4.9 |
| ADD | C-API-FRAME-003 | GET `/v1/frames/:frame_id/context` | 端点对外可用（HTTP-only：frame context JSON） | `spec.md` §4.9 |

### P1-S6 delta（Chat-2 路由与流式）

| 类型 | Contract-ID | 接口 | 变化/说明 | SSOT |
|---|---|---|---|---|
| CHANGE | C-API-CHAT-001 | POST `/v1/chat` | 运行时故障口径补齐：provider timeout（180s watchdog）、Pi crash 等需以“可恢复且可观测”的方式对外表达（HTTP 状态仍为 SSE 流式响应） | `roadmap.md` P1-S6；`spec.md` §4.6 |

### P1-S7 delta（端到端验收 / 功能冻结）

| 类型 | Contract-ID | 接口 | 变化/说明 | SSOT |
|---|---|---|---|---|
| RETAIN | (all) | `/v1/*` | P1 功能冻结：不新增/废弃/替代对外端点；以全链路回归验证为主 | `roadmap.md` P1-S7 |
