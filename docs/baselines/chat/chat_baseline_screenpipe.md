...
- 核查方法：直接阅读 screenpipe 源码（`_ref/screenpipe/`），所有结论附文件路径 + 行为描述
...
| 维度 | 说明 |
|------|------|
| Chat 响应 streaming | Pi stdout/stderr 逐行 JSON → Tauri event `pi_event` → 前端增量渲染 |
| 顶层事件类型（核心） | `message_update`, `tool_execution_start`, `tool_execution_update`, `tool_execution_end`, `agent_start`, `agent_end`, `turn_start`, `turn_end`, `message_start`, `message_end`, `response` |
| `message_update` 子事件类型（`assistantMessageEvent.type`） | `text_delta`, `thinking_start`, `thinking_delta`, `thinking_end`, `content_block_delta`（以及未来扩展子事件） |
| 扩展顶层事件（实现可观测） | `auto_retry_end` 等；Rust 层透传原始事件，前端按需消费 |
...
**证据**：`pi.rs` stdout/stderr reader 透传 `pi_event`；`standalone-chat.tsx` 分层处理 `data.type` 与 `assistantMessageEvent.type`；`pi-event-handler.ts` 顶层 `PiEventType` 定义
...
| v1.2 | 2026-03-03 | 修正流式事件口径分层：区分顶层 `data.type`（核心 11 种）与 `message_update.assistantMessageEvent.type` 子事件；补充扩展顶层事件（如 `auto_retry_end`）的前向兼容说明 |
