    ...
    def process(self, dt_boxes, rec_res):
        ...
        for i in range(1, len(items)):
            ...
            if abs(item["y_center"] - current_line_mean_y) < threshold:
                ...
                current_line_mean_y = (current_line_mean_y * (n-1) + item["y_center"]) / n
                current_line_mean_height = (current_line_mean_height * (n-1) + item["height"]) / n
            else:
                # Start new line
...
class RapidOCRBackend:
    ...
    def _initialize(self):
        ...
        logger.info(f"Initializing RapidOCRBackend (use_local={use_local})")
    ...
    def extract_text(self, image):
        ...
        try:
            ...
            if settings.debug:
                logger.debug(f"RapidOCR: Extracted {len(lines)} merged lines from image")
...
if __name__ == "__main__":
    ...
    dt_boxes = [
        ...
        [[65, 152], [90, 152], [90, 172], [65, 172]],    # Row 2: "3B" (Close gap -> should merge with space) 
        
        [[10, 200], [50, 200], [50, 220], [10, 220]],    # Row 3: "人工" 
        [[50, 200], [90, 200], [90, 220], [50, 220]],    # Row 3: "智能" (Zero gap -> should merge tightly) 
    ]
    rec_res = [
        ("GLM-4", 0.99), ("Minimind", 0.95), ("Qwen", 0.98), 
        ("DeepSeek", 0.99), ("3B", 0.96), 
        ("人工", 0.99), ("智能", 0.99) 
    ]
    ...
    assert merged_lines[0] == "GLM-4\tMinimind\tQwen", f"Row 1 Grid Mismatch: '{merged_lines[0]}'"
    
    # Check Row 2: English Space
    assert merged_lines[1] == "DeepSeek 3B", f"Row 2 English Space Mismatch: '{merged_lines[1]}'"
    
    # Check Row 3: Chinese Tight Join
    assert merged_lines[2] == "人工智能", f"Row 3 Chinese Join Mismatch: '{merged_lines[2]}'"
