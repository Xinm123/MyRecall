...
class Settings(BaseSettings):
    ...
    ocr_rapid_min_score: float = Field(
        default=0.65,
        alias="OPENRECALL_OCR_RAPID_MIN_SCORE",
        description="Minimum confidence score for RapidOCR results (0.0-1.0); lower values keep more results",
    )
    ...
    video_fps: int = Field(
        default=30, alias="OPENRECALL_VIDEO_FPS", description="Video recording FPS"
    )
    video_crf: int = Field(
        default=23,
    ...
    audio_enabled: bool = Field(
        ...
        description="Audio capture mainline is hard-disabled in Phase 2.6.",
    )
    ...
    def validate_video_pipeline_mode(cls, v: str) -> str:
        ...
        if normalized not in {"segment", "chunk_process"}:
            raise ValueError(
                "OPENRECALL_VIDEO_PIPELINE_MODE must be one of: segment, chunk_process"
            )
        return normalized
    ...
    def validate_audio_vad_sensitivity(cls, v: str) -> str:
        ...
        if normalized not in _AUDIO_VAD_SENSITIVITY_DEFAULTS:
            raise ValueError(
                "OPENRECALL_AUDIO_VAD_SENSITIVITY must be one of: low, medium, high"
            )
        return normalized

    @field_validator("audio_enabled", mode="before")
    @classmethod
    def force_audio_disabled(cls, _v) -> bool:
        """Phase 2.6 hard shutdown: ignore env toggles and keep audio off."""
        return False
    ...
    def apply_audio_vad_sensitivity_defaults(cls, values):
        ...
        if not min_ratio_explicit:
            resolved["OPENRECALL_AUDIO_VAD_MIN_SPEECH_RATIO"] = mapped[
                "min_speech_ratio"
            ]
        return resolved
    ...
    def video_monitor_id_list(self) -> list[str]:
        ...
        return [
            item.strip() for item in self.video_monitor_ids.split(",") if item.strip()
        ]
    ...
    def ensure_directories(self) -> None:
        ...
        directories = [
            ...
            # Phase 2.6: Audio directories excluded from auto-creation (audio hard shutdown)
