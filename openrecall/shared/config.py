...
class Settings(BaseSettings):
    ...
    auth_mode: str = Field(
        default="strict",
        alias="OPENRECALL_AUTH_MODE",
        description="Auth mode: strict|permissive|disabled",
    )
    device_tokens_json: str = Field(
        default="{}",
        alias="OPENRECALL_DEVICE_TOKENS_JSON",
        description="JSON map of device_id -> token config",
    )
    token_grace_seconds: int = Field(
        default=86400,
        alias="OPENRECALL_TOKEN_GRACE_SECONDS",
        description="Grace period for previous tokens",
    )
    max_upload_bytes: int = Field(
        default=10485760,
        alias="OPENRECALL_MAX_UPLOAD_BYTES",
        description="Max upload file size",
    )
    log_sensitive: bool = Field(
        default=False,
        alias="OPENRECALL_LOG_SENSITIVE",
        description="Allow sensitive data in logs",
    )
    rate_limit_upload_rps: int = Field(
        default=5,
        alias="OPENRECALL_RATE_LIMIT_UPLOAD_RPS",
        description="Upload rate limit per device",
    )
    rate_limit_chat_rps: int = Field(
        default=1,
        alias="OPENRECALL_RATE_LIMIT_CHAT_RPS",
        description="Chat rate limit per device",
    )
    idempotency_strict: bool = Field(
        default=True,
        alias="OPENRECALL_IDEMPOTENCY_STRICT",
        description="Strict idempotency mode",
    )
    legacy_device_id: str = Field(
        default="legacy",
        alias="OPENRECALL_LEGACY_DEVICE_ID",
        description="Default device ID for legacy clients",
    )
    device_id: str = Field(
        default="",
        alias="OPENRECALL_DEVICE_ID",
        description="Device identifier for this client",
    )
    device_token: str = Field(
        default="",
        alias="OPENRECALL_DEVICE_TOKEN",
        description="Auth token for this device",
    )
    ...
    def validate_auth_mode(cls, v: str) -> str:
        allowed = {"strict", "permissive", "disabled"}
        if v not in allowed:
            raise ValueError("auth_mode must be one of: strict, permissive, disabled")
        return v
    ...
    @property
    def client_timezone(self) -> str:
        """Get client timezone as IANA name."""
        import time
