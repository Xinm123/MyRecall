...
def _image_path_to_filename(image_path: str | None, fallback_ts: float) -> str:
    """Convert absolute image path to relative filename for URL construction."""
    if not image_path:
        return f"{int(fallback_ts)}.png"

    path = Path(image_path)
    screenshots_path = settings.screenshots_path

    try:
        return str(path.relative_to(screenshots_path))
    except ValueError:
        return path.name
...
class SearchEngine:
    def __init__(
        ...
        self.embedding_provider: Any = get_ai_provider("embedding")
        self.reranker = get_reranker()
        logger.info(
            f"SearchEngine initialized with Reranker: {settings.reranker_mode} ({settings.reranker_model})"
        )

    def search(
        self, user_query: str, limit: int = 50, device_id: str | None = None
    ) -> List[SemanticSnapshot]:
        sorted_results = self._search_impl(
            user_query=user_query, limit=limit, device_id=device_id
        )
        results: list[SemanticSnapshot] = []
        for item in sorted_results[:limit]:
            snapshot = cast(SemanticSnapshot, item["snapshot"])
            score = float(item["score"])
            results.append(self._attach_score(snapshot, score))
        return results
    ...
    def search_debug(
        ...
        for idx, item in enumerate(sorted_results[:limit]):
            snap = cast(SemanticSnapshot, item["snapshot"])
            dbg = cast(dict[str, Any], item["debug"])
            ...
            out.append(
                {
                    ...
                    "filename": _image_path_to_filename(snap.image_path, ts),
                    "final_rank": idx + 1,
                    "final_score": float(item["score"]),
                    "rerank_rank": dbg.get("rerank_rank"),
                    "rerank_score": float(dbg.get("rerank_score") or 0.0)
    ...
    def _search_impl(
        ...
        fts_boost_events: list[tuple[Any, ...]] = []

        vector_store = self._get_vector_store(device_id)
        ...
        if parsed.text:
            ...
            try:
                ...
                query_vec_list: List[float] = list(query_vec)
                ...
                vector_results = vector_store.search(
                    query_vec_list, limit=limit * 2, where=where_clause
                )
                ...
                if settings.debug:
                    qlen = (
                        len(query_vec_list)
                        if hasattr(query_vec_list, "__len__")
                        else None
                    )
                    logger.debug(
                    ...
                    if where_clause:
                        ...
                        unfiltered = vector_store.search(
                            query_vec_list, limit=limit * 2, where=None
                        )
                        ...
                        for item in unfiltered:
                            ...
                            if parsed.end_time and ts > parsed.end_time:
                                continue
                            vector_where_pass += 1
                        vector_where_filtered_out = (
                            vector_where_unfiltered_candidates - vector_where_pass
                        )
                        logger.debug(
                ...
                if settings.debug and parsed.text:
                    logger.debug(
                        f"🧠 Vector Search Summary | where={where_clause} | merged={len(results_map)}"
                    )
        ...
        if fts_query:
            try:
                ...
                fts_rows = self.sql_store.search(
                    fts_query, limit=limit, device_id=device_id
                )
                ...
                if settings.debug and missing_ids:
                    logger.debug(
                        f"🔤 FTS-only IDs (missing from vector) | count={len(missing_ids)}"
                    )
                    logger.debug(
                        "🔤 FTS-only IDs | "
                        + ", ".join(
                            [f"{sid}({fts_bm25_by_id.get(sid)})" for sid in missing_ids]
                        )
                    )

                if missing_ids:
                    fetched_snaps = vector_store.get_snapshots(missing_ids)
        ...
        if candidates and (parsed.text or user_query):
            try:
                doc_texts = [
                    construct_rerank_context(c["snapshot"]) for c in candidates
                ]
                    ...
                    if settings.log_sensitive:
                        try:
                            ...
                            with open(log_file, "a", encoding="utf-8") as f:
                                f.write(f"\n\n{'=' * 60}\n")
                                f.write("SENSITIVE_LOGGING_ENABLED\n")
                            ...
                            logger.debug(
                                f"🧠 Reranker Input (Full Context) logged to {log_file}"
                            )
                            logger.debug(
                                f"🧠 Reranker Input Preview (Doc 1/{len(doc_texts)}):\n{doc_texts[0]}...$$"
                            )
                            logger.debug(
                                f"🧠 Reranker Input Preview (Doc 2/{len(doc_texts)}):\n{doc_texts[1]}...$$"
                            )
                        except Exception as e:
                            logger.error(f"Failed to write rerank debug log: {e}")

                scores = self.reranker.compute_score(
                    parsed.text or user_query, doc_texts
                )
                ...
                if any(s != 0.0 for s in scores):
                    ...
                    if settings.debug:
                        logger.debug(
                            f"🧠 Reranked Top Results (Top {len(candidates)}):"
                        )
                        ...
                        for i, c in enumerate(candidates):
                            ...
                            logger.debug(
                                f"#{i + 1} Score: {c['score']:.4f} | OCR_len={ocr_len} Cap_len={caption_len} Pair_len={pair_len} |\nDoc:\n{doc_preview}...\n"
                            )
        ...
        if settings.debug:
            ...
            if fts_only_filtered_ids:
                logger.debug(
                    "FTS-only Filtered IDs | " + ", ".join(fts_only_filtered_ids)
                )
            ...
            logger.debug(
                f"🔍 Search Debug Report | q='{user_query}' | showing_top={min(limit, len(sorted_results))}"
            )
            for i, item in enumerate(sorted_results[:limit]):
                snap = item["snapshot"]
                dbg = item["debug"]
                ts = snap.context.timestamp
                img = _image_path_to_filename(snap.image_path, ts)
    ...
    def _get_vector_store(self, device_id: str | None) -> VectorStore:
        if device_id in {None, "legacy", settings.legacy_device_id}:
            return self.vector_store
        if getattr(self.vector_store, "device_id", None) == device_id:
            return self.vector_store
        return VectorStore(device_id=cast(str, device_id))

    def _attach_score(
        self, snapshot: SemanticSnapshot, score: float
    ) -> SemanticSnapshot:
