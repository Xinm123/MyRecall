...
class FramesStore:
    ...
    def insert_frame(
        ...
        """Insert a frame row, idempotent on capture_id.

        Args:
            capture_id: UUID v7 generated by the Host.
            metadata: Parsed CapturePayload dict from the multipart request.
            snapshot_path: Absolute path to the persisted JPEG file.

        Returns:
            (frame_id, is_new) — is_new=True for a new row, False for duplicate.

        Raises:
            sqlite3.Error: On any DB failure.
        """
        timestamp = metadata.get("timestamp") or metadata.get("capture_time", "")
        ...
        try:
            with self._connect() as conn:
                ...
                if is_new:
                    frame_id: int = cursor.lastrowid  # type: ignore[assignment]
                    logger.info(
                        "insert_frame: new frame capture_id=%s frame_id=%d",
                        capture_id,
                        frame_id,
                    )
                ...
                logger.debug(
                    "insert_frame: idempotent duplicate capture_id=%s frame_id=%d",
                    capture_id,
                    existing_id,
                )
            ...
            logger.error("insert_frame failed capture_id=%s: %s", capture_id, e)
            raise
