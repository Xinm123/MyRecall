...
class Context(BaseModel):
    ...
    time_bucket: str = Field(
        description="Time bucket for partitioning, e.g., '2024-01-24-10'"
    )
...
class Content(BaseModel):
    ...
    keywords: List[str] = Field(
        default_factory=list, description="Extracted entities and keywords"
    )
    scene_tag: str = Field(
        default="", description="Scene classification, e.g., 'coding'"
    )
    action_tag: str = Field(
        default="", description="Action classification, e.g., 'debugging'"
    )
...
class SemanticSnapshot(LanceModel):
    ...
    context: Context
    content: Content
    embedding_vector: Vector(1024) = Field(description="The fusion text embedding")  # type: ignore
