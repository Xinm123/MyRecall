...
class RapidOCRBackend:
    ...
    def _initialize(self):
        ...
        if use_local:
            ...
            det_size_mb = _osp.getsize(det_path) / (1024 * 1024)
            rec_size_mb = _osp.getsize(rec_path) / (1024 * 1024)
            logger.info(
                f"Loading ONNX models (DET={det_size_mb:.1f}MB, REC={rec_size_mb:.1f}MB) "
                f"— server models may take 1-3 min on CPU, please wait..."
            )
            logger.info(f"  DET: {det_path}")
            logger.info(f"  REC: {rec_path}")
            logger.info(f"  CLS: {cls_path}")

            use_gpu = settings.device in ("cuda", "mps")
            ...
            logger.info("RapidOCRBackend ONNX models loaded successfully")
        else:
            use_gpu = settings.device in ("cuda", "mps")
            logger.info("Loading RapidOCR with auto-download models...")
            self.engine = RapidOCR(use_angle_cls=True, use_gpu=use_gpu)
            logger.info("RapidOCRBackend initialized successfully")
    ...
    def extract_text(self, image):
        ...
        try:
            ...
            result = [
                item for item in result if item[2] >= settings.ocr_rapid_min_score
            ]
            if not result:
                return ""
