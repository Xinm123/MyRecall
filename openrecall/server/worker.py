...
class ProcessingWorker(threading.Thread):
    ...
    def _process_task(
        ...
        try:
            ...
            if task.image_relpath:
                image_path = settings.screenshots_path / task.image_relpath
            else:
            ...
            if settings.debug:
                ...
                if len(text) > 0 and settings.log_sensitive:
                    preview_text = text[:100].replace("\n", " ")
                    logger.debug(
                        f"  SENSITIVE_LOGGING_ENABLED OCR Preview: {preview_text}..."
                    )
            ...
            if settings.fusion_log_enabled and settings.log_sensitive:
                try:
                    log_dir = Path("logs")
                    ...
                    with open(log_file, "a", encoding="utf-8") as f:
                        f.write(
                            f"\n--- Task #{task.id} [{datetime.datetime.now()}] SENSITIVE_LOGGING_ENABLED ---\n"
                        )
