...
**Configuration:** TOML files (`server-local.toml`, `client-local.toml`, `client-remote.toml`). `--config=/path` takes precedence over `--mode`. All settings are in `myrecall/shared/config.py`.
...
Chat API routes: `/chat/api/*` — see `myrecall/client/chat/routes.py` for all endpoints.

**Entry:** `python -m myrecall.client`

### Edge (Server) — `myrecall.server`
...
**Entry:** `python -m myrecall.server`
...
1. Migration: `myrecall/server/database/migrations/YYYYMMDDHHMMSS_description.sql`
2. Update `FramesStore` in `myrecall/server/database/frames_store.py`
3. Update API in `myrecall/server/api_v1.py`
4. Update tests: `tests/test_p1_s1_frames.py`
5. Run: `pytest tests/test_v3_migrations_bootstrap.py`

### Add a Hot-Reloadable Setting

1. Default in `ClientSettingsStore.DEFAULTS` (`myrecall/client/database/settings_store.py`)
2. Validator in `myrecall/client/web/routes/settings.py` validators dict
3. Getter in `myrecall/client/runtime_config.py` (SQLite > TOML)
...
1. Update `CaptureTrigger` enum in `myrecall/client/events/base.py`
2. Add detection in `myrecall/client/events/`
3. Update `myrecall/client/recorder.py`
...
All v1 endpoints defined in `myrecall/server/api_v1.py`. Chat endpoints in `myrecall/client/chat/routes.py`. Legacy `/api/*` returns 410 Gone.

### Environment Variables

All env vars and defaults defined in `myrecall/shared/config.py`. Key ones: `MYRECALL_SERVER_DATA_DIR`, `MYRECALL_CLIENT_DATA_DIR`, `MYRECALL_PORT`, `MYRECALL_DEBUG`, `MYRECALL_AI_PROVIDER`, `MYRECALL_DEVICE`.
