...
class TestSingletonReplacement:
    ...
    def test_server_singleton_replaced(self):
        """After __main__ loads, myrecall.shared.config.settings should be ServerSettings."""
        import myrecall.shared.config

        original = myrecall.shared.config.settings

        from myrecall.server.config_server import ServerSettings

        new_settings = ServerSettings._from_dict({})
        myrecall.shared.config.settings = new_settings

        try:
            assert myrecall.shared.config.settings.server_host == "0.0.0.0"
            assert myrecall.shared.config.settings.port == 8083
            assert myrecall.shared.config.settings.device == "cpu"
            assert myrecall.shared.config.settings.debug is False
        finally:
            myrecall.shared.config.settings = original

    def test_client_singleton_replaced(self):
        """After __main__ loads, myrecall.shared.config.settings should be ClientSettings."""
        import myrecall.shared.config
        ...
        try:
            assert myrecall.shared.config.settings.debug is False
            assert (
                myrecall.shared.config.settings.api_url == "http://localhost:8083/api"
            )
            assert myrecall.shared.config.settings.click_debounce_ms == 3000
        finally:
            myrecall.shared.config.settings = original
